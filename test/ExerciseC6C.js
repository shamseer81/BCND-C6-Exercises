...
contract('ExerciseC6C', async (accounts) => {
  ...
  it('can register Employee, add sale and calculate bonus', async () => {
    ...
    // ACT
    await config.exerciseC6CApp.registerEmployee(employee.id, employee.isAdmin, employee.address);
    await config.exerciseC6CApp.addSale(employee.id, 400);
    let bonus = await config.exerciseC6CApp.getEmployeeBonus.call(employee.id);
